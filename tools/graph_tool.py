...
def init_graph_store() -> GraphStore:
    """Initialize the shared GraphStore singleton."""
    global _store
    if _store is None:
        _store = GraphStore(
...
def get_graph_store() -> GraphStore:
    """Get the shared GraphStore singleton."""
    if _store is None:
        raise RuntimeError("GraphStore not initialized. Call init_graph_store() first.")
    return _store
